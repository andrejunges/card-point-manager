...
var fs = require('fs'),
    options = {},
    env = process.env.NODE_ENV || 'developmenta';
...
if (env == 'development') {
  ...
  (function () {
    var debug = function () {
      ...
      if (first) framework.emit('debug-start');else framework.emit('debug-restart');
    };
    ...
    isDebugging = process.argv.indexOf('debugging') !== -1;
    directory = process.cwd();
    path = require('path');
    first = process.argv.indexOf('restart') === -1;
